...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === "panel") {
      ...
      await interaction.reply({ content: "✅ Panel wurde erfolgreich gepostet!", ephemeral: true });
    }
    ...
    if (interaction.commandName === "giveaway-start") {
      ...
      const pingMsg = await interaction.channel.send({ content: `<@&${GIVEAWAY_PING_ROLE_ID}>` });
      const msg = await interaction.channel.send({ embeds: [giveawayEmbed], components: [row] });
      await pingMsg.delete().catch(() => null);

      await interaction.reply({ content: "🎉 Giveaway wurde gestartet!", ephemeral: true });
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === "giveaway_join") {
      ...
      const updatedEmbed = new EmbedBuilder()
        .setTitle(data.gewinn)
.setDescription(Endet: <t:${endZeitUnix}:R> (<t:${endZeitUnix}:F>)\nGehostet von: <@${data.hostId}>\nTeilnahmen: **${data.teilnehmer.length}**\nGewinner: **${data.gewinnerAnzahl}**\n\nGiveaway #${data.giveawayId}).setColor(0x2F3136);await interaction.update({ embeds: [updatedEmbed] });return;}await interaction.deferReply({ ephemeral: true });const channels = interaction.guild.channels.cache;const hasTicket = channels.some(channel =>channel.type === ChannelType.GuildText &&channel.permissionOverwrites.cache.has(interaction.user.id) &&(channel.name.includes("kauf-") || channel.name.includes("verkauf-") || channel.name.includes("support-") || channel.name.includes("claim-")));if (hasTicket) {return interaction.editReply({ content: "❌ Du hast bereits ein offenes Ticket! Schließe dieses zuerst, bevor du ein neues öffnest." });}let ticketName = "ticket";let welcomeMessage = "Willkommen im Ticket!";let categoryId = null;if (interaction.customId === "spawner_kaufen") {ticketName = 🛒-kauf-${interaction.user.username};welcomeMessage = Hallo ${interaction.user}, hier kannst du **Spawner kaufen**. Bitte schreibe, welche Spawner du suchst und wie viele du benötigst!;categoryId = CATEGORIES.kaufen;} else if (interaction.customId === "spawner_verkaufen") {ticketName = 💰-verkauf-${interaction.user.username};welcomeMessage = Hallo ${interaction.user}, hier kannst du **Spawner verkaufen**. Bitte nenne uns deine Spawner und die genaue Anzahl. Der Ankauf erfolgt zu unseren festen Server-Preisen!;categoryId = CATEGORIES.verkaufen;} else if (interaction.customId === "support_ticket") {ticketName = 🛠️-support-${interaction.user.username};welcomeMessage = Hallo ${interaction.user}, ein Teammitglied wird sich gleich um dein **Support-Anliegen** kümmern. Bitte beschreibe dein Problem genau.;categoryId = CATEGORIES.support;} else if (interaction.customId === "giveaway_claim") {ticketName = 🎉-claim-${interaction.user.username};welcomeMessage = Herzlichen Glückwunsch ${interaction.user}! Du möchtest deinen **Giveaway-Gewinn einfordern**. Bitte sende einen Screenshot des Gewinns hier hinein.;categoryId = CATEGORIES.giveaway;}try {const ticketChannel = await interaction.guild.channels.create({name: ticketName,type: ChannelType.GuildText,parent: categoryId,permissionOverwrites: [{ id: interaction.guild.roles.everyone.id, deny: [PermissionsBitField.Flags.ViewChannel] },{ id: interaction.user.id, allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.ReadMessageHistory] },{ id: client.user.id, allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.ManageChannels] }],});const ticketEmbed = new EmbedBuilder().setTitle("✉️ Support-Ticket geöffnet").setDescription(welcomeMessage).setColor(0x5865F2).setTimestamp();await ticketChannel.send({ embeds: [ticketEmbed] });await interaction.editReply({ content: Dein Ticket wurde erfolgreich erstellt: ${ticketChannel} });} catch (error) {console.error(error);await interaction.editReply({ content: "Fehler beim Erstellen deines Tickets." });}}});client.login(TOKEN);
